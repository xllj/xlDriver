        ...
        */
        ...
        public static bool INIWriteValue(string section, string key, string value)
        {
            try
            {
                if (FileName != null)
                {
                    return INIWriteValue(FileName, section, key, value);
                }
                return false;
            }
            catch (Exception)
        ...
        /// <summary>  
        /// 在INI文件中，删除所有节点中的所有内容。  
        /// </summary>  
        /// <param name="iniFile">INI文件</param>  
        /// <param name="section">节点</param>  
        /// <returns>操作是否成功</returns>  
        public static bool INIEmptySection(string iniFile)
        {
            try
            {
                string[] sections = xlIni.INIGetAllSectionNames(iniFile);
                foreach (string s in sections)
                {
                    if (!INIEmptySection(iniFile, s))
                    {
                        return false;
                    }
                }
        ...
        public static void Test()
        {
            try
            {
                string file = ".\\CH112Config.ini";

                //写入/更新键值  
                xlIni.INIWriteValue(file, "Desktop", "Color", "Red");
                xlIni.INIWriteValue(file, "Desktop", "Width", "3270");

                xlIni.INIWriteValue(file, "Toolbar", "Items", "Save,Delete,Open");
                xlIni.INIWriteValue(file, "Toolbar", "Dock", "True");

                //写入一批键值  
                xlIni.INIWriteItems(file, "Menu", "File=文件\0View=视图\0Edit=编辑");

                //获取文件中所有的节点  
                string[] sections = xlIni.INIGetAllSectionNames(file);

                //获取指定节点中的所有项  
                string[] items = xlIni.INIGetAllItems(file, "Menu");

                //获取指定节点中所有的键  
                string[] keys = xlIni.INIGetAllItemKeys(file, "Menu");

                //获取指定KEY的值  
                string value = xlIni.INIGetStringValue(file, "Desktop", "color", null);

                //删除指定的KEY  
                //xlIni.INIDeleteKey(file, "desktop", "color");

                //删除指定的节点  
                //xlIni.INIDeleteSection(file, "desktop");

                //清空指定的节点  
                //xlIni.INIEmptySection(file, "toolbar");
            }
            catch (Exception)
            {
        ...
        public static void Test1()
        {
            try
            {
                string file = ".\\TestDefault.ini";

                xlIni.INIEmptySection(file);
                string s;
                for (int i = 0; i < 26; i++)
                {
                    //写入/更新键值 
                    s = Convert.ToChar(i + 'a').ToString();
                    xlIni.INIWriteValue(file, s, "is", "T");
                    xlIni.INIWriteValue(file, s, "Inf", "");
                }
                //xlIni.INIEmptySection(file);
                //写入一批键值  
                //xlIni.INIWriteItems(file, "Menu", "File=文件\0View=视图\0Edit=编辑");
